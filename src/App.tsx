...
function App() {
  ...
  const adicionarHabilidade = (habilidadesString: string) => {
    ...
    const habilidadesDigitadas = habilidadesString.split(",").map((hab) => ({
      ...
      nome: hab.trim(),
      nivel: "Nenhum", // Mantemos o nível 'Nenhum' como padrão para habilidades digitadas em massa
    }));

    setListaDeHabilidades((listaAnterior) => [...listaAnterior, ...habilidadesDigitadas]);
    setHabilidadeTemp("");
  };
  ...
  const handleExportTXT = () => {
    ...
    const habilidadesTxt = listaDeHabilidades.length
      ? listaDeHabilidades
          .map((h, i) => `${i + 1}. ${h.nome} ${h.nivel !== "Nenhum" ? `(${h.nivel})` : ''}`)
          .join("\n")
  ...
  const handleExportWord = () => {
    const experienciasHtml = experiencias.length
      ? experiencias
          .map(
            (exp) =>
              `<li>Empresa: ${exp.empresa} | Cargo: ${exp.cargo} | Descrição: ${exp.descricao} | Período: ${exp.inicio} - ${
                exp.fim || "Atual"
              }</li>`
          )
          .join("")
    ...
    const habilidadesHtml = listaDeHabilidades.length
      ? `<ul>${listaDeHabilidades
          .map((h) => `<li>${h.nome} ${h.nivel !== "Nenhum" ? `(${h.nivel})` : ''}</li>`)
          .join("")}</ul>`
  ...
  const handleExportPDF = () => {
      ...
      ? `<div class="${PreviewStyles.section}">
            ...
            <div class="${PreviewStyles.habilidadesList}">
                ${listaDeHabilidades
                  .map(
                    (h) =>
                      `<span class="${PreviewStyles.skillTag}">${h.nome} ${h.nivel !== "Nenhum" ? `(${h.nivel})` : ''}</span>`
                  )
                  .join("")}
            </div>
        </div>`
    ...
    const previewContent = `
        <div class="${PreviewStyles.personalInfo}">
            ...
            <p class="${PreviewStyles.jobTitle}">${
          ...
          experiencias.length > 0
            ...
            ${experiencias
              .map(
                ...
                <p><strong>Período:</strong> ${exp.inicio} - ${
